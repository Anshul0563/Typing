...
import { paragraphSchema, idSchema } from '../validators/schemas.js';

export const paragraphRouter = Router();
paragraphRouter.use(authenticate, authorize('admin'));
paragraphRouter.get('/', paragraphs.listParagraphs);
paragraphRouter.post('/', validate(paragraphSchema), paragraphs.createParagraph);
paragraphRouter.put('/:id', validate(paragraphSchema), paragraphs.updateParagraph);
