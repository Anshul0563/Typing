...
export const startTestSchema = z.object({ params: idParams, body: z.object({ paragraphId: z.string().regex(/^[a-f\d]{24}$/i) }) });
export const submitSchema = z.object({ body: z.object({ testToken: z.string().min(40), paragraphId: z.string().regex(/^[a-f\d]{24}$/i), typedText: z.string().max(30000), testMode: z.enum(['TCS', 'NTA', 'Standard']).default('Standard'), totalKeystrokes: z.coerce.number().int().min(0).max(100000).default(0), backspaceCount: z.coerce.number().int().min(0).max(100000).default(0) }) });
