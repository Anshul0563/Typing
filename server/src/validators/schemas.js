...
export const examSchema = z.object({
  body: z.object({ name: z.string().trim().min(2).max(100), organization: z.string().trim().min(2).max(100), language: z.enum(['English', 'Hindi']), durationMinutes: z.coerce.number().int().min(1).max(120), paragraphLength: z.coerce.number().int().min(1), category: z.enum(examCategories), logo: z.string().trim().regex(/^(\/assets\/exams\/[a-z0-9-]+\.svg|data:image\/(?:svg\+xml|png|jpeg|webp);base64,[A-Za-z0-9+/=]+)$/, 'Choose a valid local exam icon or upload a supported image'), scoringRule: z.object({ mode: z.literal('standard-word'), errorPenalty: z.coerce.number().min(0.1).max(10) }), status: z.enum(['active', 'inactive']).default('active'), description: z.string().trim().max(240).default('') })
});
