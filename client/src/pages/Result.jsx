...
function ErrorColumn({ title, total, categories, breakdown, severity, ssc }) {
  return <article className={`error-column error-${severity}`}><header><span className={`error-swatch compare-${severity}`} /><div><h3>{title}: {total}</h3><p>{ssc && severity === 'half' ? 'Not applicable in SSC Stenographer mode' : severity === 'half' ? 'Each error carries 0.5 weight' : 'Each error carries full weight'}</p></div></header><div>{categories.map((key) => <span key={key}><small>{labels[key]}</small><strong>{breakdown[key] ?? 0}</strong></span>)}</div></article>;
}
...
export default function Result() {
  ...
  return <div className="result-page">
    <section className="result-hero-card"><div className="result-hero-copy"><span className="result-check"><CheckCircle2 /></span><div><p className="eyebrow">Test submitted</p><h1>{result.exam?.name || 'Typing test'} result</h1><p>{ssc ? 'SSC Stenographer evaluation: every detected mistake counts as one full error.' : 'Practice evaluation: full errors count as 1 and half errors count as 0.5.'}</p></div></div><div className="result-hero-score"><span>{result.testMode || 'Standard'} Mode</span><strong>{result.netWpm ?? 0}</strong><small>Net WPM</small></div></section>
    ...
    <section className="formula-card"><div><span>Formula used</span><h2>{ssc ? 'SSC Stenographer formula' : 'Typing Practice formula'}</h2></div><p><strong>Accuracy:</strong> ((Reference characters − {ssc ? 'total errors' : 'weighted errors'}) ÷ Reference characters) × 100</p><p><strong>Weighted errors:</strong> {ssc ? 'Every mistake × 1' : 'Full errors + (Half errors × 0.5)'}</p><p><strong>Gross WPM:</strong> (Typed characters ÷ 5) ÷ Time in minutes</p><p><strong>Net WPM:</strong> Gross WPM − (Weighted errors × Penalty ÷ Time in minutes)</p></section>
