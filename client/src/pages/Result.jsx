...
function HighlightedText({ parts, emptyLabel, highlightSeverity, highlightCategories, highlightTone }) {
  if (!parts?.length) return <p className="comparison-empty">{emptyLabel}</p>;
  return <p className="comparison-text">{parts.map((part, index) => {
    const highlighted = part.severity !== 'correct' && (part.severity === highlightSeverity || highlightCategories?.includes(part.category));
    return highlighted
      ? <mark key={index} className={`compare-${highlightTone || part.severity}${part.missing ? ' compare-missing' : ''}`} title={`${labels[part.category] || 'Error'}${part.missing ? ' (missing)' : ''}`}>{part.text}</mark>
...
export default function Result() {
  ...
  return <div className="result-page">
    ...
    <section className="result-comparison"><div className="result-section-heading"><span>Character-accurate review</span><h2>Original Text vs Your Typed Text</h2><p>Original Text highlights full errors and omissions; Your Typed Text highlights half errors. Symbols mark missing spaces, punctuation or line breaks.</p></div><div className="comparison-card-grid"><article className="comparison-card"><header><FileText /><h3>Original Text</h3><small className="comparison-card-key"><i className="compare-full" />Full errors</small></header><HighlightedText parts={comparison.referenceParts} emptyLabel="Reference text unavailable." highlightSeverity="full" highlightTone="full" /></article><article className="comparison-card"><header><Keyboard /><h3>Your Typed Text</h3>{!ssc && <small className="comparison-card-key"><i className="compare-half" />Half errors</small>}</header><HighlightedText parts={comparison.typedReviewParts || comparison.typedParts} emptyLabel="No text was typed." highlightSeverity={ssc ? 'full' : 'half'} highlightCategories={ssc ? undefined : halfCategories} highlightTone={ssc ? 'full' : 'half'} /></article></div><div className={`comparison-legend ${ssc ? 'single-legend' : ''}`}><div><span><i className="compare-full" />Full Error</span><p>{ssc ? 'All detected mistakes—including spacing, capitalization, punctuation, transposition and paragraphic errors—count as full errors.' : 'Omissions · Additions · Spellings · Repetitions · Substitutions · Incomplete Words'}</p></div>{!ssc && <div><span><i className="compare-half" />Half Error</span><p>Spacing · Capitalization · Punctuation · Transposition · Paragraphic</p></div>}</div></section>
