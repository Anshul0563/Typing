...
export default function TypingTest() {
  ...
  const referenceTokens = useMemo(() => {
    ...
    return (data?.paragraph?.content.match(/\s+|\S+/gu) || []).map((text) => {
      ...
      return { text, isWord, wordIndex: isWord ? wordIndex++ : null };
    });
  }, [data]);
  const activeWordIndex = useMemo(() => {
    const wordsStarted = typed.match(/\S+/gu)?.length || 0;
    return Math.max(0, wordsStarted - (/\S$/u.test(typed) ? 1 : 0));
  }, [typed]);
