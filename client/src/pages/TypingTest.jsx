...
export default function TypingTest() {
  ...
  const [data, setData] = useState(null); const [typed, setTyped] = useState(''); const [phase, setPhase] = useState('instructions'); const [seconds, setSeconds] = useState(0); const [error, setError] = useState('');
  ...
  const submit = useCallback(async () => {
    ...
    try {
      const response = await api('/results', { method: 'POST', body: JSON.stringify({ testToken: testTokenRef.current, paragraphId: data.paragraph._id, typedText: typedRef.current, totalKeystrokes: keystrokesRef.current, backspaceCount: backspacesRef.current }) });
  ...
  const settingsLocked = !['instructions', 'ended'].includes(phase);
