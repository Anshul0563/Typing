...
export default function Exams() {
  ...
  return <><div className="student-page-title exams-page-title"><div><span>Test catalogue</span><h1>Choose your typing test</h1><p>Browse every active exam and select the passage you want to practise.</p></div><span className="exam-count">{query ? `${filtered.length} found` : `${exams.length} active`}</span></div><Notice>{error}</Notice>{filtered.length ? <div className="student-exam-grid exams-catalogue-grid">{filtered.map((exam) => <ExamCard key={exam._id} exam={exam} onStart={() => navigate(`/test/${exam._id}`)} />)}</div> : <section className="results-empty compact-empty"><Keyboard /><h2>{exams.length ? 'No matching exams' : 'No active exams'}</h2><p>{exams.length ? 'Try another exam name, organization, category or language.' : 'Your administrator is preparing new tests. Please check again shortly.'}</p></section>}</>;
}
